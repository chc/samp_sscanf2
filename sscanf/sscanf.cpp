...
bool
	DoK(AMX * amx, char ** defaults, char ** input, cell * cptr, bool optional, int kustom_extra);
	...
	n_sscanf(AMX * amx, cell * params)
{
	...
	char *
		string;
	STR_PARAM(amx, params[2], string);
	// Pointer to the current format specifier.
	char *
		format;
	STR_PARAM(amx, params[3], format);
		...
		else
		{
			switch (*format++)
			{
				...
				case 'K':
					...
					{
						amx_GetAddr(amx, params[paramPos++], &cptr);
						if (DoK(amx, &format, &string, cptr, true, kustom_extra))
						{
							break;
						}
					}
					...
					{
						// Pass a NULL pointer so data isn't saved anywhere.
						if (DoK(amx, &format, &string, NULL, true, kustom_extra))
						{
							break;
						}
					}
				...
				case 'k':
					...
					{
						amx_GetAddr(amx, params[paramPos++], &cptr);
						if (DoK(amx, &format, &string, cptr, false, kustom_extra))
						{
							break;
						}
					}
					...
					{
						// Pass a NULL pointer so data isn't saved anywhere.
						if (DoK(amx, &format, &string, NULL, false, kustom_extra))
						{
							break;
						}
					}
		...
		else
		{
			...
			switch (*format++)
			{
				...
				case 'K':
					...
					{
						amx_GetAddr(amx, params[paramPos++], &cptr);
						if (DoK(amx, &format, NULL, cptr, true, kustom_extra))
						{
							break;
						}
					}
					else
					{
						...
						// Also pass NULL data so it knows to only collect the
						// default values.
						if (DoK(amx, &format, NULL, NULL, true, kustom_extra))
						{
							break;
						}
					}
